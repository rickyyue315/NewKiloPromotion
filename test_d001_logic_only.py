...
def create_test_data():
    ...
    test_data_a = {
        ...
        "Site": ["HA01", "D001"],
        "RP Type": ["RF", "RF"],
        "SaSa Net Stock": [10, 30],  # D001 has 30, others have 10
        "Pending Received": [5, 10],
        "Safety Stock": [20, 30],
        "Last Month Sold Qty": [30, 25],
        "MOQ": [6, 6],
        "Supply source": [2, 2],
    ...
    test_data_b1 = {
        ...
        "Target Type": ["ALL", "HK"],
        "Promotion Days": [7, 5],
        "Target Cover Days": [7, 0]
    }
    
    # File B Sheet2 test data
    test_data_b2 = {
        "Site": ["HA01", "D001"],
        "Shop Target(HK)": [0.3, 0.2],
        "Shop Target(MO)": [0.4, 0.3],
        "Shop Target(ALL)": [0.5, 0.4]
    }
    
    return test_data_a, test_data_b1, test_data_b2

def test_d001_shortage_logic():
    """Test D001 stock shortage alert logic without Chinese characters"""
    ...
    for _, row in summary.iterrows():
        ...
        # Check if alert is correctly set
        if (expected_shortage and shortage_alert == expected_shortage) or (not expected_shortage and not shortage_alert):
