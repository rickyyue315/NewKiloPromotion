...
4. **派貨建議階段**：
   - 應用HB87-RF特殊邏輯（如適用）
   - 應用MOQ規則
   - 計算建議派貨量（Suggested_Dispatch_Qty）
   - 計算建議DN量（Suggested_DN_Qty）
   - 計算Target Dispatch
   - 確定派貨類型（Dispatch_Type）
   - 生成派貨備註（Dispatch_Remark）
...
如果 Is_Promo_SKU 為 True:
    Site_Promo_Demand = SKU_Target × Site_Target_% / Promotion_Days × Effective_Target_Cover_Days
否則:
    Site_Promo_Demand = 0
```
- `Is_Promo_SKU`：必須同時滿足SKU_Target > 0和Site_Target_% > 0
- `Site_Target_%`：根據Target_Type選擇對應的門市目標百分比
- `Promotion_Days`：推廣天數，如果為0則使用1作為除數（避免除以零）
- `Effective_Target_Cover_Days`：有效目標覆蓋天數（見上述公式說明）
...
#### 8. HB87-RF特殊邏輯
```
如果 Site = "HB87" 且 RP_Type = "RF" 且 Site_Promo_Demand <= 0:
    # 沒有 Target，使用特殊公式計算
    raw_value = Safety_Stock - SaSa_Net_Stock - Pending_Received
    
    如果 raw_value <= 0 或 MOQ <= 0:
        Suggested_Dispatch_Qty = 0
    否則:
        # 使用 Mround (四捨五入到最接近的 MOQ 倍數)
        Suggested_Dispatch_Qty = max(round(raw_value / MOQ) × MOQ, MOQ)
    
    # 如果 Suggested_Dispatch_Qty > 0，則在 Dispatch_Remark 中顯示標識
    如果 Suggested_Dispatch_Qty > 0:
        Dispatch_Remark = "HB87-RF派貨"
```

#### 9. 建議派貨量 (Suggested_Dispatch_Qty)
```
# HB87-RF特殊邏輯（最高優先級）
如果 Site = "HB87" 且 RP_Type = "RF" 且 Site_Promo_Demand <= 0:
    raw_value = Safety_Stock - SaSa_Net_Stock - Pending_Received
...
# ND站點邏輯
如果 RP_Type = "ND" 且 Site_Promo_Demand > 0:
    Suggested_Dispatch_Qty = ceil(Site_Promo_Demand / MOQ) × MOQ
否則:
    Suggested_Dispatch_Qty = 0
...
#### 10. Target Dispatch (Target_Dispatch)
```
Target_Dispatch = Mround((Site_Promo_Demand - SaSa_Net_Stock - Pending_Received), MOQ)
```
- `Site_Promo_Demand`：門市推廣目標需求量
- `Mround(x, MOQ)`：四捨五入到最接近的MOQ倍數，實現為 `round(x / MOQ) × MOQ`
- 如果計算結果 ≤ 0 或 MOQ ≤ 0，則 Target_Dispatch = 0
- 確保結果不小於 MOQ

#### 11. 建議DN量 (Suggested_DN_Qty)
```
# ND站點邏輯
如果 RP_Type = "ND" 且 Site_Promo_Demand > 0:
    # 先計算基礎派貨量（與Suggested_Dispatch_Qty相同）
    base_qty = ceil(Site_Promo_Demand / MOQ) × MOQ
    
    # 根據Promotion Days決定是否限制在50
    如果 Promotion_Days > 4:
        如果 base_qty <= 50:
            Suggested_DN_Qty = base_qty
        否則:
            # 找到最大的MOQ倍數且 <= 50
            Suggested_DN_Qty = floor(50 / MOQ) × MOQ
    否則:
        # Promotion Days <= 4，使用base_qty
        Suggested_DN_Qty = base_qty
    
    Suggested_DN_Qty = round(Suggested_DN_Qty)
否則:
    Suggested_DN_Qty = 0
...
如果 RP_Type = DISPATCH_RP_TYPE ("RF") 且 Net_Demand_for_Dispatch > 0 且 MOQ > 0:
    # 先計算基礎派貨量
    base_qty = ceil(max(Net_Demand_for_Dispatch, MOQ) / MOQ) × MOQ
...
**注意**：HB87-RF特殊邏輯不計算Suggested_DN_Qty，僅計算Suggested_Dispatch_Qty

#### 12. 派貨類型 (Dispatch_Type)
```
# 新SKU邏輯檢查（最高優先級）
如果 Launch Date為空白且 Suggested_DN_Qty > 0:
    Dispatch_Type = "新SKU必須由Buyer首次派貨"
...
- Total_Target_Dispatch：總Target Dispatch數量
...
- 如果 有效庫存 ≥ Total_Demand 且 D001 SaSa_Net_Stock > 100件：庫存足夠，RP team會安排Lot For Lot
- 如果 有效庫存 ≥ Total_Demand 但 D001 SaSa_Net_Stock < 100件：庫存足夠目標數量，但D001少於100件，在需要時進行搓貨
- 如果 有效庫存 < Total_Demand：庫存不足夠，請Buyer留意

**當Supply source為1或4時：**
- 如果 有效庫存 ≥ Total_Demand：庫存足夠
- 如果 有效庫存 < Total_Demand：庫存不足夠，請Buyer開PO
...
| 列名 | 類型 | 說明 | 預設值 |
|------|------|------|--------|
| Launch Date | 文本 | 產品上市日期 | 空白（表示新SKU） |
...
- Launch Date為空白或空值時將被識別為新SKU，觸發特殊處理邏輯
...
- Safety_Stock：安全庫存
- Last_Month_Sold_Qty_capped：上月銷售量（已截斷）
- Daily_Sales_Rate：日銷售率
- Effective_Target_Cover_Days：有效目標覆蓋天數
- Base_Demand：基本需求
- Site_Promo_Demand：門市推廣需求
- Total_Demand：總需求
- Net_Demand_raw：淨需求原始值
- Net_Demand_for_Dispatch：派貨淨需求
- MOQ：最小訂購量
- Promotion_Days：推廣天數
- Promo_Target_Cover_Days：目標覆蓋天數（Cover Days）
- Suggested_Dispatch_Qty：建議派貨量
- Suggested_DN_Qty：建議DN量
- Target_Dispatch：Target Dispatch數量（基於推廣目標需求量計算）
- Dispatch_Type：派貨類型
- Dispatch_Remark：派貨備註（如"HB87-RF派貨"、"ND 派貨"）
- Launch_Date：產品上市日期
...
- Group_No：推廣組別
- Article：產品編碼
- Article Description：產品描述（如果存在）
- Product Hierarchy：產品層級（如果存在）
- Article Long Text (60 Chars)：產品長描述（如果存在）
- Description p. group：產品組描述（如果存在）
- Total_Demand：總需求
- Total_Stock_Available：總可用庫存
- Total_Stock：總庫存
- Total_Pending：總在途貨品
- Total_Dispatch：總建議派貨量
- Total_Suggested_DN_Qty：總建議DN量
- Total_Target_Dispatch：總Target Dispatch數量
- D001_SaSa_Net_Stock：DC淨庫存
- Effective_Inventory：有效庫存（D001 + H字頭店鋪庫存 + H字頭店鋪待收貨）
- Enhanced_Inventory_Status：增強庫存狀態（基於供應來源和庫存情況的詳細判斷，包含D001庫存不足提示）
- Inventory_Difference：庫存差額（Effective_Inventory - Total_Demand，負數在Excel中顯示為黃格紅字粗體）
- New_SKU_Alert：新SKU提示（Launch Date為空白且Suggested_DN_Qty>0時顯示"新SKU必須由Buyer首次派貨"）
- D001_Stock_Shortage_Alert：D001庫存不足提示（當D001庫存不足以應對Total_Suggested_DN_Qty或Total_Target_Dispatch時顯示）

### 關鍵結果解釋

#### Dispatch_Type（派貨類型）
- **新SKU必須由Buyer首次派貨**：Launch Date為空白且Suggested_DN_Qty>0（最高優先級）
- **D001**：DC門市，特殊處理
...
#### Dispatch_Remark（派貨備註）
- **HB87-RF派貨**：HB87站點且RP_Type為RF且Site_Promo_Demand <= 0（無Target）時的特殊派貨
- **ND 派貨**：ND站點且有派貨需求（Suggested_Dispatch_Qty > 0 或 Suggested_DN_Qty > 0）時的備註

#### Target_Dispatch（Target Dispatch）
- **計算公式**：`Mround((Site_Promo_Demand - SaSa_Net_Stock - Pending_Received), MOQ)`
- **用途**：基於推廣目標需求量計算派貨量，考慮現有庫存和在途貨品
- **Mround規則**：四捨五入到最接近的MOQ倍數
- **結果**：如果計算結果 ≤ 0 或 MOQ ≤ 0，則 Target_Dispatch = 0

#### Enhanced_Inventory_Status（增強庫存狀態）
**當Supply source為2時：**
- **庫存足夠, RP team會安排Lot For Lot**：Total_Demand ≤ 有效庫存 且 D001庫存 > 100件
- **庫存足夠目標數量, 但D001少於100件, 在需要時進行搓貨**：Total_Demand ≤ 有效庫存 但 D001庫存 < 100件
- **庫存不足夠, 請Buyer留意**：Total_Demand > 有效庫存

**當Supply source為1或4時：**
- **庫存足夠**：Total_Demand ≤ 有效庫存
- **庫存不足夠, 請Buyer開PO**：Total_Demand > 有效庫存
...
**D001庫存不足提示：**
- 當 D001_SaSa_Net_Stock < Total_Suggested_DN_Qty 或 D001_SaSa_Net_Stock < Total_Target_Dispatch 時
- 在 Enhanced_Inventory_Status 中添加提示："D001 not enough for RP team to add dispatch"
- 在 D001_Stock_Shortage_Alert 欄位中單獨顯示此提示

#### Inventory_Difference（庫存差額）
- 計算公式：Effective_Inventory - Total_Demand
- 正數表示庫存有餘量
- 負數表示庫存不足（在Excel中會以黃格紅字粗體顯示）
...
#### Q12: HB87站點的RF類型為什麼會有不同的計算邏輯？
**原因**：HB87站點的RF類型在沒有推廣目標（Site_Promo_Demand <= 0）時使用特殊計算公式
**解決方案**：
1. 這是預期行為，HB87-RF無Target時使用公式：Safety_Stock - SaSa_Net_Stock - Pending_Received
2. 結果會四捨五入到最接近的MOQ倍數
3. 會在Dispatch_Remark欄位顯示"HB87-RF派貨"標識
4. 此邏輯在計算Suggested_Dispatch_Qty時優先級最高

#### Q13: 什麼是"新SKU必須由Buyer首次派貨"？
**原因**：Launch Date為空白且Suggested_DN_Qty>0的SKU被識別為新SKU
**解決方案**：
1. 這是最高優先級的派貨類型，會覆蓋其他所有邏輯
2. 新SKU需要Buyer特別處理首次派貨
3. 如需更改此行為，請填寫Launch Date欄位
...
```
**說明**：
- H字頭店鋪包括HA*、HB*、HC*、HD*開頭的門市代碼
- 有效庫存用於判斷庫存狀態和是否需要補貨
- 與Total_Demand比較得出Inventory_Difference
...
3. **優化Suggested_DN_Qty計算**：
   - ND站點：使用Site_Promo_Demand計算基礎派貨量，然後根據Promotion Days決定是否限制在50
   - RF站點：使用Net_Demand_for_Dispatch計算基礎派貨量，然後根據Promotion Days決定是否限制在50
   - 當Promotion Days > 4時，上限為50件（但必須是MOQ的倍數）
   - 當Promotion Days <= 4時，使用完整的基礎派貨量
   - 結果四捨五入至整數
4. **增強報告功能**：
...
7. **新增Target Dispatch功能**：
   - 在Detail_Calculation中新增Target_Dispatch欄位
   - 使用公式：`Mround((Site_Promo_Demand - SaSa_Net_Stock - Pending_Received), MOQ)`
   - 在Summary_Report中新增Total_Target_Dispatch欄位，計算總Target Dispatch數量
8. **新增HB87-RF特殊邏輯**：
   - 當HB87站點且RP_Type為RF且無Target（Site_Promo_Demand <= 0）時，使用特殊計算公式
   - 公式：`Mround((Safety_Stock - SaSa_Net_Stock - Pending_Received), MOQ)`
   - 在Dispatch_Remark中顯示"HB87-RF派貨"標識
9. **新增D001庫存不足提示**：
   - 當D001_SaSa_Net_Stock不足夠Total_Suggested_DN_Qty或Total_Target_Dispatch時
   - 在Enhanced_Inventory_Status中添加提示："D001 not enough for RP team to add dispatch"
   - 在Summary_Report中新增D001_Stock_Shortage_Alert欄位
10. **修正Site_Promo_Demand計算公式**：
    - 更新公式為：`Site_Promo_Demand = SKU_Target × Site_Target_% / Promotion_Days × Effective_Target_Cover_Days`
    - 當Promotion_Days = 0時，使用1作為除數（避免除以零）
    - 使用Effective_Target_Cover_Days（已包含默認值處理）而非Promo_Target_Cover_Days
    - 確保計算結果準確，符合業務需求
11. **在Detail_Calculation中新增Promotion Days和Cover Days欄位**：
    - 新增Promotion_Days欄位：顯示推廣天數，影響Site_Promo_Demand和Suggested_DN_Qty的計算
    - 新增Promo_Target_Cover_Days欄位：顯示目標覆蓋天數（Cover Days），影響Site_Promo_Demand的計算
    - 這兩個欄位讓用戶能清楚看到推廣期間和覆蓋天數對需求計算的影響
    - 確保計算更精準，便於追蹤和審計
12. **更新文檔以反映實際系統邏輯**：
    - 修正Suggested_Dispatch_Qty的計算邏輯說明，明確HB87-RF特殊邏輯的優先級
    - 修正Suggested_DN_Qty的計算邏輯說明，明確ND和RF站點的不同計算方式
    - 確保文檔與實際程式碼一致
