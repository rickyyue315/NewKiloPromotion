...
"""
Test New SKU Logic Fix - ASCII version
"""
...
def test_new_sku_logic():
    ...
    try:
        ...
        new_sku_message = "New SKU must be dispatched by Buyer first time"
        for idx, row in detail.iterrows():
            ...
            # Use ASCII-safe print to avoid encoding issues
            print(f"  {article}: Launch_Date='{launch_date}', DN_Qty={dn_qty}, Dispatch_Type='{dispatch_type.encode('ascii', 'replace').decode('ascii')}'")
            
            # Verify new SKU logic
            if (launch_date == '' or launch_date is None) and dn_qty > 0:
                if dispatch_type != new_sku_message:
                    print(f"    [ERROR] {article} should show new SKU message")
                else:
        ...
        for idx, row in summary.iterrows():
            ...
            # Use ASCII-safe print to avoid encoding issues
            print(f"  {article}: Enhanced_Inventory_Status='{enhanced_status.encode('ascii', 'replace').decode('ascii')}'")
            print(f"    New_SKU_Alert='{new_sku_alert.encode('ascii', 'replace').decode('ascii')}'")
