...
def test_specific_issue():
    ...
    # 創建模擬的 File A 數據 - 測試特定問題
    file_a_data = {
        'Article': ['TEST001', 'TEST002', 'TEST003'],
        'Site': ['ND001', 'ND002', 'ND003'],
        'RP Type': ['ND', 'ND', 'ND'],
        'Supply source': [2, 2, 2],
        'SaSa Net Stock': [50, 30, 20],
        'Pending Received': [0, 0, 0],
        'Safety Stock': [5, 5, 5],
        'Last Month Sold Qty': [0, 0, 0],
        'MOQ': [10, 10, 10],
    }
    ...
    file_b1_data = {
        ...
        'Article': ['TEST001', 'TEST002', 'TEST003'],
        'SKU Target': [0, 0, 0],  # 全部為 0
        'Target Type': ['ALL', 'ALL', 'ALL'],
        'Promotion Days': [5, 5, 5],
    }
    
    # 創建模擬的 File B Sheet2 數據
    file_b2_data = {
        'Site': ['ND001', 'ND002', 'ND003'],
        'Shop Target(HK)': [0, 0, 0],
        'Shop Target(MO)': [0, 0, 0],
        'Shop Target(ALL)': [0, 0, 0],
    }
    ...
    problem_cases = detail[
        ...
        (detail['Dispatch_Type'].str.contains('需生成|開PO', na=False))
    ]
    
    if not problem_cases.empty:
        print(f"Found {len(problem_cases)} ND sites with Suggested_DN_Qty=0 but wrong Dispatch_Type")
        for _, row in problem_cases.iterrows():
            print(f"  Site: {row['Site']}")
            print(f"  Suggested_DN_Qty: {row['Suggested_DN_Qty']}")
            print(f"  Dispatch_Type: {row['Dispatch_Type']}")
            print(f"  Site_Promo_Demand: {row['Site_Promo_Demand']}")
            print(f"  SKU_Target: {row['SKU_Target']}")
            print(f"  Site_Target_%: {row['Site_Target_%']}")
    else:
        print("No ND sites with Suggested_DN_Qty=0 and wrong Dispatch_Type found")
